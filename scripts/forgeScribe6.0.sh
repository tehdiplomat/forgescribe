...
currentdir=`pwd`
cd /NMT/
python -m nmt.nmt --ckpt=${model}/best_bleu/translate.ckpt-34000 --out_dir=${model} --src=or --tgt=sc --inference_input_file=/tmp/tokenedoracle --inference_output_file=/tmp/tokenedscript --vocab_prefix=./${model}/forge.vocab
